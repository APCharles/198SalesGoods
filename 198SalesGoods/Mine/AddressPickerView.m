...
- (void)loadAddressData{
    ...
    NSDictionary *provinceDic = [self dictionaryWithJsonString:provinceStr];
    
    NSDictionary *cityDic = [self dictionaryWithJsonString:cityStr];
    
  
    NSArray *proArr = [provinceDic allKeys];
    
    
  
    
    for (int i = 0; i < [provinceDic count] ; i ++) {
        NSDictionary *proDic = [provinceDic objectForKey:[NSString stringWithFormat:@"%d",i + 1]];
        
        [_provincesArr addObject:proDic];
    }
    

    _pArr         = [[NSMutableArray alloc]init];
    
    NSArray *cit = [cityDic allValues];
        ...
        for (int i = 0 ;i < proArr.count; i++) {
            ...
            for (NSDictionary *dic in cit) {
                
                if ([dic[@"ProvinceID"] isEqualToString:[NSString stringWithFormat:@"%d",i + 1]]) {
...
- (NSString *)pickerView:(UIPickerView *)pickerView
    ...
    if (0 == component) {
        ...
        NSDictionary *dic = selectP.cities[row];
        return dic[@"CityName"];
    }
...
- (void)sureBtnClicked{
        ...
        NSDictionary *cityDic = p.cities[selectCity];
        
        [_delegate sureBtnClickReturnProvince:p.name City:cityDic];
